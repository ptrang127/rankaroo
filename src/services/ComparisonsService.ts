...
async function incrementComparisonByFirstSubjectIdAndSecondSubjectId(firstSubjectId: number, secondSubjectId: number, voteId: number): Promise<Comparison> {
  ...
  const firstId = Math.min(firstSubjectId, secondSubjectId);
  const secondId = Math.max(firstSubjectId, secondSubjectId);
  ...
  const comparison: Comparison = (comparisonRecord == null) ?
